...
/*   Updated: 2022/07/14 06:14:51 by aaljaber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*nw_readtxt(char *readtxt)
{
	...
	string = (char *)malloc(sizeof(char) * (gnl_strlen(readtxt) - i + 1));
	if (!string)
		return (NULL);
	i += 1;
	j = 0;
...
static char	*reading(int fd, char *readtxt)
{
	...
	while (!gnl_strchr(readtxt, '\n') && bytes != 0)
	{
		...
		readtxt = gnl_strjoin(readtxt, buffer);
	}
	free(buffer);
	return (readtxt);
}
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	readtxt = reading(fd, readtxt);
	if (!readtxt)
		return (NULL);
	line = lineout(readtxt);
	readtxt = nw_readtxt(readtxt);
	return (line);
}
